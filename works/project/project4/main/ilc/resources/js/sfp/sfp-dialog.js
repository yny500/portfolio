...
sfp.dialog = (function($) {
	...
	var addlayer = function(type, style, params, closedFn, cancelFn) {
	...
	, open = function(_url, style, params, closedFn, cancelFn, _window) {
		...
		if(url.indexOf('.') == -1){
			let _suffix = top.location.href.split('.').reverse()[0];
			if(_suffix.indexOf('mobile') != -1){
				url = url +".mobiledialog";
			}else if(_suffix.indexOf('pop') != -1){
				url = url +".popdialog";
			}else if(_suffix.indexOf('pda') != -1){
				url = url +".pdadialog";
			}else if(_suffix.indexOf('tablet') != -1){
				url = url +".tabletdialog";
			}else{
				url = url +".dialog";
			}
		}
