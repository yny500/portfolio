...
$.widget( "sfp.template", {
	...
	_bindEvent : function(){
		...
		this.element.on("keyup.s3s change.s3s", "[data-summary-target]", function(e){
			var target = $(e.target).data("summary-target");
			var total = 0;
	...
	summary : function(){
		var selector = "body " + this.elTagClass + ":visible:not(.D)";
	...
	removeRowIndex : function(idx){
		var $card = this.tbody.find(this.elTagClass).eq(idx);
		this._remove($card);
	},
	...
	_postProcess : function($card){
		$card.find("[data-template-checked], [data-date-format], [data-float-fixed], [data-number-comma]").each(function(){
			...
			if($this.is("[data-date-format]") && $.trim($this.html()) != ""){
				...
				var format = $this.data("floatFixed");
	...
	_merge : function($card, data, callbackFn){
		var $el = this._generateCard(data, callbackFn, { crud : this.CRUD.U });
...
//	}
	...
	_generateCard : function(data, callbackFn, params){//row, crud, cbFn, templateId){ // TODO card draw 완료후에 호출이 필요하다.
		...
		var $card = this._templateTag(rowData,
	...
	_noDataGenerateCard : function(callbackFn, params){
		var $card = this._templateTag({}, params.templateId === undefined ? this.template[this.elementId] : this.template[params.templateId], params.crud);
		var colspanLength = $card.find("td").length;
	...
	_templateTag : function(rowData, tmp, crud){
		var $card = $(tmp.replace(/@{([a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣|0-9|_\s-/]+[.]?)+}/g, function(val){
