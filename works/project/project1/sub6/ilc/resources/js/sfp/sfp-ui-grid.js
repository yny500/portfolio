...
//					if (ctrlDown && (e.keyCode == cKey)) console.log("Ctrl+C");
			...
			$.each(list, function(index, row){
				...
				$.each(row, function(j, col){
					...
					if(typeof col === "string"){
						...
						var colspan = col.colspan === undefined ? 1 : col.colspan;
						if($.isArray(col.width) && col.width.length != colspan){
							col.width = col.width.slice(0, col.width.length-1);
						}
						target == $this.thead && index == 0 && $this._addcol($th, colspan, col.width);

						col.colspan !== undefined && $th.attr("colspan", col.colspan);
						col.rowspan !== undefined && ($th.attr("rowspan", col.rowspan), maxRow=col.rowspan);
						col.id !== undefined && $th.attr("id", row[j].id);
						col.summary !== undefined && $th.attr("data-summary-name",col.summary);
						$th.html(col.name);
						$th.addClass(col.classes)
						col.sort == false && $th.attr("data-row-sort", false);
						col.property && $th.attr("headerProperty", col.property);
					}
					$tr.append($th);
				});
	...
	_addcol : function($th, rec, width){
		...
		let w = (width === undefined ? '*': $.isArray(width) ? width[width.length-rec] : width);
		this.colgroup.append($("<col>", {"style" : "width:" + (w == '*' && window.locationType == "mobile" ? '100px' : w)}));
	...
	getParent : function(target){
		const $card = $(target).closest(this.elTagClass);
		const groupId = $card.data("grid-group-id");
	...
	getParentData : function(target){
		...
		$card = $card.hasClass("grid-main-row") ? $card :
			this.tbody.find(".grid-main-row[data-grid-group-id="+groupId+"]");
		return this.getData($card);
	},

	getDataListCRUD : function(cbFn){
		if(this.options.isSubMode){
			const _this = this; //this.options.isSubMode &&
			const result = this.getDataList(cbFn);
	...
	drawPage : function(list, paging, callbackFn, params){
		this.draw(list, callbackFn, params);
		this.setPage(paging);
		return this;
	},
	...
	_draw : function(list, cbFn, setting){
		...
		if(setting.isSub){
			const $main = this.tbody.find(".grid-main-row[data-grid-group-id=" + setting.groupId + "]");
	...
	_setScroll : function(){
		const scroll = this.options.isScrollY && this.body.height() < this.tbody.height() ? "scroll" : "hidden";
	...
	sort : function(idx , order){ // order =>  내림 차순 -1, 오름차순 = 1
		...
		const rows = this.tbody.find("tr").get();
	...
	_appendCheckBox : function($tr, data, params){
		if(this.options.selectedCheckBox != null){
			if((this.options.isSubMode && params.isSub) || $tr.data("crud-mode") == this.CRUD.C.mode){
				...
				const $td = $("<td><label><input type='checkbox' class='grid-tr-checkbox' data-skip-change-event data-grid-disabled-fix><span class='check-mark'></span></label></td>")
	...
	_generateCard : function(data, callbackFn, params){
		...
		const $card = this._super(data, callbackFn, params);
		if(this.options.isSubMode){
